...
function App() {
  ...
  const fetchItems = async (q: string) => {
    ...
    try {
      ...
      const [ebayRes, leboncoinRes] = await Promise.all([
      ...
      // Show error only if both sources failed
      if (!ebayData?.success && !leboncoinData?.success) {
  ...
  return (
    <div style={{padding: 16, maxWidth: 960, margin: '0 auto'}}>
      ...
      {!loading && !error && ebayItems.length === 0 && leboncoinItems.length === 0 && (
        <div>Aucun résultat.</div>
      )}

      <div style={{paddingTop: 8, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16}}>
